...
def test_parse_date_format_valid():
    ...
    datetime.strftime(datetime.now(), Date().parse("YYYY YY m MM M DDD DD \\ /"))


def test_get_date_from_exif():
    assert Date().from_exif({"CreateDate": "2017-01-01 01:01:01"}) == {
...
def test_get_date_from_custom_date_field():
    assert Date().from_exif(
        {"CustomField": "2017:01:01 01:01:01"}, date_field="CustomField"
    ) == {"date": datetime(2017, 1, 1, 1, 1, 1), "subseconds": ""}


def test_get_date_from_exif_strip_timezone():
    assert Date().from_exif({"CreateDate": "2017-01-01 01:01:01-02:00"}) == {
        "date": datetime(2017, 1, 1, 1, 1, 1),
        "subseconds": "",
    }
...
def test_get_date_from_exif_strip_timezone_sub_sec():
    assert Date().from_exif({"SubSecCreateDate": "2019:10:06 11:02:50.575+01:00"}) == {
...
def test_get_date_from_exif_subseconds():
    assert Date().from_exif({"CreateDate": "2017-01-01 01:01:01.20"}) == {
        "date": datetime(2017, 1, 1, 1, 1, 1),
        "subseconds": "20",
    }
...
def test_get_date_from_exif_invalid():
    assert Date().from_exif({"CreateDate": "Invalid"}) == {
        "date": None,
        "subseconds": "",
    }
